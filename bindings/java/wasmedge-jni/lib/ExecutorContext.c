 ...
 */
JNIEXPORT jobject JNICALL Java_org_wasmedge_ExecutorContext_instantiate__Lorg_wasmedge_StoreContext_2Lorg_wasmedge_ASTModuleContext_2
...
JNIEXPORT void JNICALL Java_org_wasmedge_ExecutorContext_invoke
        (JNIEnv * env, jobject thisObject, jobject jStoreContext, jobject jFuncInstanceContext, jobject jParams, jobject jReturns) {
    WasmEdge_ExecutorContext *exeCxt = getExecutorContext(env, thisObject);
    ...
    WasmEdge_FunctionInstanceContext* function = getFunctionInstanceContext(env, jFuncInstanceContext);

    uint32_t returnLen = GetReturnLen(function);
    WasmEdge_Value *returns = malloc(sizeof(WasmEdge_Value) * returnLen);

    WasmEdge_Result result = WasmEdge_ExecutorInvoke(exeCxt, function, wasm_params, paramLen, returns, returnLen);
...
JNIEXPORT void JNICALL Java_org_wasmedge_ExecutorContext_registerImport
    ...
    WasmEdge_Result result = WasmEdge_ExecutorRegisterImport(exeCxt, storeCxt, impObj);
    handleWasmEdgeResult(env, &result);
}

JNIEXPORT jobject JNICALL Java_org_wasmedge_ExecutorContext_register
        (JNIEnv *env, jobject thisObject, jobject jStore, jobject jAstCxt, jstring jModName) {
    WasmEdge_ExecutorContext *exeCxt = getExecutorContext(env, thisObject);
    WasmEdge_StoreContext *storeCxt = getStoreContext(env, jStore);
    ...
    WasmEdge_Result result = WasmEdge_ExecutorRegister(exeCxt, &instCxt, storeCxt, astCxt, wModName);
    handleWasmEdgeResult(env, &result);
